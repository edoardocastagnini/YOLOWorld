...
# --- LAMP head-wise per C2fAttn ---
def _lamp_scores(vals: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    """Implementazione LAMP: normalizza ogni valore con sqrt(cumsum) sul vettore ordinato decrescente.
       vals: (H,) valori non-negativi (energie per-head). Ritorna gli score rimappati all'ordine originale."""
    s, order = torch.sort(vals, descending=True)        # energia decrescente
    csum = torch.cumsum(s, dim=0)                       # somma cumulativa
    scr_sorted = s / torch.sqrt(csum + eps)             # LAMP score
    scores = torch.empty_like(scr_sorted)
    scores[order] = scr_sorted                          # rimappa all'indice originale
    return scores

def zero_c2fattn_heads_LAMP(model, head_ratio: float, log_file: Path, p: int = 2, include_bn: bool = False):
    """
    Azzera intere head nei blocchi C2fAttn usando LAMP head-wise.
    - head_ratio in [0,1)
    - p: norma per l'energia di ciascuna head (tipicamente 2)
    - include_bn: se True, aggiunge |gamma| della BN della proj_conv nella stima d'energia per-head
    """
    assert 0.0 <= head_ratio < 1.0
    lines = []

    for name, mod in model.named_modules():
        if not isinstance(mod, C2fAttn):
            continue

        attn = mod.attn  # MaxSigmoidAttnBlock
        ...
        # Pesi della proiezione (out = nh*hc)
        W = attn.proj_conv.conv.weight.data  # shape [nh*hc, inC, k, k]
        c2 = W.shape[0]
        assert c2 == nh * hc, f"Expected nh*hc={nh*hc}, got {c2}"

        # Energia per head (L2/Frobenius su tutti i filtri della head)
        head_energy = []
        for h in range(nh):
            start, end = h * hc, (h + 1) * hc
            w_h = W[start:end, ...].reshape(-1)
            e = torch.norm(w_h, p=p)
            if include_bn:
                bn = getattr(attn.proj_conv, 'bn', None)
                if isinstance(bn, nn.BatchNorm2d):
                    # aggiungi contributo BN gamma/beta sui canali della head (facoltativo, leggero)
                    g = bn.weight.data[start:end].abs()
                    # somma delle gamma come “peso” additivo
                    e = e + g.sum()
            head_energy.append(e)
        head_energy = torch.stack(head_energy)  # (nh,)

        # LAMP: normalizza con sqrt(cumsum) sull'ordinamento per energia ↓
        lamp = _lamp_scores(head_energy)  # (nh,)

        # Seleziona le k head con score più basso
        heads_to_zero = torch.topk(lamp, k=k, largest=False).indices.tolist()

        # Indici canali out da azzerare (blocchi contigui da hc canali)
        out_idx = []
        for h in heads_to_zero:
            out_idx.extend(range(h * hc, (h + 1) * hc))
        out_idx = torch.tensor(out_idx, dtype=torch.long)

        # Azzeramento pesi/bias + BN (come nel tuo flow attuale)
        with torch.no_grad():
        ...
        # Logging per il tuo file maschere
        lines.append(f"{name}.attn.proj_conv.conv|out|{out_idx.tolist()}")

        # (Opzionale) log accoppiamenti se ec/gl hanno stessa ampiezza di c2
...
def get_unique_run_dir(base_dir="runs/detect/FIXED_H0.7_ALL_BB+DETECT_LAMP/prune_run"):
    i = 0
...
def prune(args):
    ...
    for i in range(args.iterative_steps):
       ...
       # pruner.step()
        ...
        zero_c2fattn_heads_LAMP(model.model, head_prune_rate, RUN_DIR / "zeroed_params.txt")
...
def prune_zeroed_weights(args):
   ...
   # path = Path("runs/detect/prune_run_2/zeroed_weights_finetune/weights/last.pt")
    ...
    for i in range(args.iterative_steps):
        ...
        #log_file = Path("runs/detect/prune_run_2/zeroed_params.txt")
        ...
        #pruned_model_path = Path("runs/detect/prune_run_2/pruned_model.pt")
        model.save(pruned_model_path)
        #save the state dict
        pruned_state_dict_path = RUN_DIR / "pruned_model_dict.pt"
        #pruned_state_dict_path = Path("runs/detect/prune_run_2/pruned_model_dict.pt")
        torch.save(model.model.state_dict(), pruned_state_dict_path)
        del pruner
